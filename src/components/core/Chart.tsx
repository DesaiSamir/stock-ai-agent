...
const StockChart: React.FC<ChartProps> = ({ 
    ...
    const ema200 = ema()
        ...
        .merge((d: ChartData, c: number) => { d.ema200 = c; })
    ...
    const candelFillColor = (data: CandleData, seriesData: CandleData[] = []) => {
        ...
        let fillColor = data.close > data.open ? "#ffffff00" : previous && (data.close >= previous.close) ? "#26a69a" : "#ef5350";
    ...
    const candelStrokeColor = (data: CandleData, seriesData: CandleData[] = []) => {
        const previous = seriesData[data.idx.index - 1];
        ...
        const strokeColor = data.close > data.open 
